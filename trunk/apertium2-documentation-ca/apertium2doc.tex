...
apareix amb el requeriment de la implementació de traductors per a parells
de llengües menys emparentats que els que fins ara s'han tractat.
...
mit\-jan\-çant regles definides per l'usuari. Aquest model l'anomenem
\textit{transferència sintàctica superficial}. Una visió
...
Amb aquesta nova estructura de nivells es pretén que hi haja un
...
el terme \textit{segment} (en anglès \textit{chunk}), per a
...
\item \texttt{transfer} (intrachunk): identifica els segments, realitza
  ...
  segment (per exemple, per a establir-hi la concordança). A més, crea
  els segments perquè siguen tractats pel mòdul següent.
\item \texttt{interchunk}: aquest mòdul rep els segments construïts
  per l'\texttt{in\-tra\-chunk} i permet reordenar-los,
  modificar la ``informació sintàctica'' associada a cada segment i, finalment,
  imprimir els segments en l'ordre nou i amb les característiques
  noves en l'eixida, creant segments nous si és necessari.
...
\item $F$ és el \emph{pseudolema del segment}, i és una cadena que
  ...
  les paraules dins del segment que estiga enllaçada a aquests
  paràmetres.
\item $W=b_{0}w_{1}b_{1}w_{2}b_{2} \ldots w_{k}b_{k}$ és la seqüència de
  paraules $w_i$ enviada pel transfer,
\notavisible{Ací cal canviar \emph{transfer} per una altra denominació més
adequada} amb els \textit{superblancs}
  $b_i$ intercalats. Aquestes paraules es troben en el mateix format
  d'Apertium, és a dir, una paraula individual
  $w_i=$\verb!^!$l_{i}E_{i}$\verb!$! conté lema $l_i$ i etiquetes $E_i$, algunes
  de les quals poden ser \emph{referències o enllaços a les etiquetes} del segment i
...
\label{ss:majuscules}
Per a cada segment, la informació de majúscules i minúscules de les paraules ve determinada
per la informació de majúscules i minúscules del
...
\item Si la primera lletra del pseudolema està en majúscules i la
  ...
  resulte de tots els possibles canvis d'ordre dels mots del segment estiga en majúscules.
...
Una requisit és que les paraules de l'interior del segment no
...
la majúscula, si n'hi ha. Aquesta tasca correspon al mòdul
\texttt{trans\-fer\--in\-tra\-chunk} 
\notavisible{Ací no hauria de dir \texttt{intrachunk} només?}
i es fa mitjançant una macro o
...
\item Una sèrie de declaracions dels \emph{atributs} que volem
  ...
  és invariable en nombre). Si dins de la regla, fora del patró, s'ha de
  ...
  s'han de definir també atributs per a aquestes.
\item Una sèrie de declaracions de variables \emph{globals}, que
  s'usen per a transferir valors d'atributs actius dins de cada
  regla o d'una regla a les quals s'apliquen posteriorment.
\item Una secció de \textit{definició de llistes de cadenes},
  generalment llistes de lemes, que seran utilitzats per a buscar-hi
  un valor que correspongui per a realitzar una transformació
  determinada.
\item Una sèrie de declaracions de \emph{macroinstruccions}; les
  ...
  freqüents i es poden incloure en diverses regles (per exemple, una
...
\texttt{lu} ---que indica que les paraules implícites han de deixar-se
intactes, i que funcionarà com un transfer d'Apertium 1.0--- o
\texttt{chunk} ---que indica que es crearà d'ofici un segment que
encapsule la paraula no reconeguda perquè puga ser tractada pels
següents mòduls en Apertium 2. El valor implícit és \texttt{lu}.
...
Aquest element té dos usos diferenciats segons en el mòdul en què s'use.
...
determinat conjunt de formes lèxiques. S'han de definir tantes
categories diferents com tipus de formes lèxiques es vulga detectar en
els patrons. Així, si ens interessa detectar tots els noms (cas de
l'\texttt{intrachunk}) o tots els sintagmes nominals de determinat tipus (cas
de l'\texttt{interchunk}) per a realitzar operacions amb ells, crearem una
...
\texttt{cotxe<n><m><sg>} per al cas de l'\texttt{intrachunk}; l'ús en
el mòdul \texttt{interchunk} seria anàleg però amb pseudoparaules.
\notavisible{Atenció a la terminologia \emph{pseudo-}}
En canvi, quan darrere del símbol utilitzat no pot venir cap altre
símbol, no s'inclou l'asterisc: per exemple,
\texttt{tags}=\texttt{"}\texttt{adv"} abastarà tots els adverbis.
També pot utilitzar-se l'asterisc per a indicar l'existència de
símbols precedents: \texttt{tags}=\texttt{"*.f.*"} inclou a totes les
formes lèxiques femenines, siguin de la categoria que siguin. A més,
pot utilitzar-se un atribut opcional, \texttt{lemma}, per a definir
formes lèxiques a partir del seu lema (veure figura
\ref{fig:cat-item}).
...
referència al nom de la regla, 
\notavisible{De la regla o del patró?}
sense etiquetes, ja que al mòdul
\texttt{postchunk} només s'usa el pseudolema per a la detecció. En la
detecció no s'hi distingeix entre majúscules i minúscules, perquè el pseudolema s'usa per a transmetre la
informació sobre les majúscules i les minúscules del segment.
...
obligatori\texttt{n} que s'utilitzarà per a referir-s'hi
dins de les regles. Les variables contenen cadenes que descriuen
...
Aquest element serveix per a donar nom a cada llista de cadenes
...
Se'n pot veure un exemple d'ús en la figura \ref{fig:deflist}.
...
(\texttt{<\textbf{action}>}) associada  que s'executa quan és
detectat.
...
bloc de zero o més instruccions de tipus \texttt{\%sentence}:
\texttt{<\textbf{choose}>} (processament condicional),
\texttt{<\textbf{let}>} (assignació de valors),
\texttt{<\textbf{out}>} (escriptura de formes lèxiques de la
...
realitzades dintre un patró, s'assignen valors a atributs del patró,\notavisible{OK?}
i, si escau, a variables globals o d'estat i s'envia la informació del
patró en LM resultant al següent mòdul (el generador morfològic).

\subsection{Element de concatenació de cadenes \\\texttt{<append>}}

La instrucció \texttt{<\textbf{append}>} pot servir per emmagatzemar
...
\subsection{Element d'invocació de macroinstrucció
  \\\texttt{<call-macro>}}
\notavisible{Crec que cal canviar totes les aparicions de \emph{cridada} a \emph{invocació}, canviant la preposició (crec)}

Dins d'una regla es pot invocar qualsevol de les macroinstruccions
definides en \texttt{<\textbf{section-def-macros}>}. Per a això, cal
especificar el nom de la macroinstrucció en l'atribut \texttt{n} i un
o més arguments en l'element de paràmetres
\texttt{<\textbf{with-param}>} (veure a continuació).

\subsection{Element de paràmetres \texttt{<with-param>}}

Aquest element s'utilitza dintre de les invocacions de
macroinstruccions \texttt{<\textbf{call-macro}>}. L'atribut
\texttt{pos} de cada argument s'utilitza per a referir-se a una forma
lèxica de la regla que invoca la macroinstrucció. Per exemple, si s'ha
definit una macroinstrucció de 2 paràmetres que realitza operacions de
concordança nom--adjectiu, pot utilitzar-se amb els arguments 1 i 2 en
una regla de nom--adjectiu, amb els arguments 2 i 3 en una regla de
...
d'adjectiu--nom. Es pot veure un exemple d'invocació de macroinstrucció
...
\end{alltt}
\end{small}
\caption{Invocació de la macroinstrucció \texttt{f-concord2} per a fer
...
\texttt{<\textbf{modify-case}>}, \texttt{<\textbf{call-macro}>} o \texttt{<\textbf{append}>},
...
instruccions (de tipus \texttt{\%sentence}:
\texttt{<\textbf{choose}>}, \texttt{<\textbf{let}>},
\texttt{<\textbf{out}>}, \texttt{<\textbf{modify-case}>},
\texttt{<\textbf{call-macro}>} i \texttt{\textbf{append}>}) que han de
realitzar-se si no és certa la condició descrita en cap dels
\texttt{<\textbf{when}>} d'un\texttt{<\textbf{choose}>}.
...
com per una senzilla condició d'igualtat de cadenes (\texttt{<\textbf{equal}>}), de començament de cadena (\texttt{<\textbf{begins-with}>}), de final de cadena (\texttt{<\textbf{ends-with}>}), de subcadena \texttt{<\textbf{contains-substring}>} o d'inclusió en conjunt \texttt{<\textbf{in}>}.
...
\item L'element de conjunció \texttt{<\textbf{and}>} representa una
  ...
  totes les condicions que s'hi inclouen són certes. Es pot veure un
  exemple del seu ús en la figura \ref{fig:regla}
\item L'element de disjunció \texttt{<\textbf{or}>} representa una
  condició, composta de dues o més condicions, que es compleix quan
  almenys una  és certa. En la figura \ref{fig:ornot} es
...
\item L'operador condicional més simple 
...
El resultat de la recerca és cert si el valor es troba
  en la llista. En aquesta comparació es pot utilitzar també
  l'atribut \texttt{caseless}: si s'hi assigna el valor \texttt{yes},
...
\item \texttt{pos} és un índex (1, 2, 3, etc.) utilitzat per a
  seleccionar una forma lèxica dins del patró associat a la regla: es correspon amb el
...
mostrats en el següent apartat.
...
\subsection{Preprocessament sense mòdul de tria lèxica}
...
Com a exemple, les tres primeres entrades que es mostren en la secció
...
\subsection{Preprocessament amb mòdul de tria lèxica}
...
\section[DTD del mòdul prechunk]{DTD del mòdul de transferència
  estructural (pre\-chunk)}
\label{ss:dtdtransfer}

DTD per al format de les regles de transferència estructural. Aquesta
definició ve amb el paquet \texttt{apertium} (versió 2.0) que es
