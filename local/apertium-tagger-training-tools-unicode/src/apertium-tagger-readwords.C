...
int main(int argc, char* argv[]) {
  ...
  if((tsxfile=="") && (probfile=="")) {
    cerr<<"Error: You did not provide a tagger specification file, neither a tagger parameter file. Use --tsxfile or --probfile to provide one of this files\n";
    help(argv[0]);
    exit(EXIT_FAILURE);
  }

  if((tsxfile!="") && (probfile!="")) {
    cerr<<"Error: You provided a tagger specification file and a tagger parameters file. Only one of this files can be provided, but not both\n";
    help(argv[0]);
    exit(EXIT_FAILURE);
  }
