...
el caso de los pares de lenguas emparentadas
...
apartado \ref{apertium2}, el sistema de transferencia avanzada, el
cual funciona con tres módulos de transferencia, con lo que la
...
identificación de patrones en el sistema avanzado: se pretende que los patrones
...
sintàctica superficial d'Apertium. De fet aquest mòdul pot
utilitzar-se individualment com a sistema de transferència sintàctica
superficial, o en combinació amb els altres dos mòduls de
transferencia per a construir un sistema de transferència sintàctica
avançada.
...
\item Eixida: 
\begin{itemize}
\item[-] si s'utilitza en un sistema de transferència avançada: segments (chunks),
  perquè el mòdul següent els detecte i realitze operacions amb ells.
\item[-] si s'utilitza en un sistema de transferència superficial: formes lèxiques que constitueixen
l'entrada del mòdul de generació 
\end{itemize}
...
manera idèntica a Apertium \notavisible{MG: caldria canviar les
  referències a 'Apertium' com si fos una altra cosa}. El nom del
fitxer de patrons tindrà l'extensió \texttt{.t1x}. Com que el
\texttt{chunker} és qui fa la consulta del diccionari bilingüe, aquest
(en forma compilada) també s'ha d'aportar al programa.
...
el mòdul de transferència actual d'Apertium \notavisible{ídem sobre 'Apertium', canviar-ho}. El programa
...
\item Especificació de l'acció implícita en l'element arrel
  ...
  funcione a la manera d'Apertium 1 (transferència sintàctica
  superficial) o d'Apertium 2 (transferència sintàctiva avançada),
  respectivament.
...
\item Generació de segments (chunks): l'etiqueta \texttt{<chunk>} és una etiqueta d'un nivell més alt
  ...
  especificades a la secció anterior; té els següents atributs:
  \begin{itemize}
...
\texttt{chunker}, \texttt{<transfer>}, té un
atribut opcional \texttt{default} que pot prendre els valors
\begin{itemize}
\item \texttt{chunk}: quan el mòdul funcioni en Apertium 2 (transferència
sintàctica avançada), per tal que es generin
  segments trivials amb les paraules que el mòdul no puga incloure en la
  formació de cap segment o que no hagen sigut especificats. El nou segment es crearà
  mit\-jan\-çant la traducció de la paraula pel diccionari bilingüe.
  El lema fix dels chunks creats implícitament és
  \texttt{default}.
\item \texttt{lu} (valor implícit): quan el mòdul funcioni com a únic
  mòdul de transferència en Apertium 1 (transferència sintàctica
  superficial), traduint la paraula amb el diccionari bilingüe.
\end{itemize}

A continuació es mostra el segment generat automàticament amb una forma lèxica no
detectada per cap regla  del
\texttt{chunker} quan l'atribut \texttt{default} té el valor \texttt{chunk}:
