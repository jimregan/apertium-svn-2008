...
\notavisible{ATENCIÓN, TEXTO TRADUCIDO AUTOMÁTICAMENTE, PENDIENTE DE REVISIÓN}

\nota{Intentar evitar el lenguaje sexista en todo el documento:
\emph{usuario}, etc.}
Cuando la persona usuaria quiera inserir nuevas entradas a un diccionario tiene que conectarse mediante un cliente navegador a la dirección donde se haya instalado el servidor de formularios,
 por ejemplo:
\begin{center}
\texttt{http://xixona.dlsi.ua.se/forms}
\end{center}
Se abrirá una página web con el portal de entrada a \texttt{Opentrad\- Apertium\- Insertion\- Form}. En el marco de la izquierda hay enllaços para obtener más \textit{información} , \textit{descargar} los programas y \textit{contacto} para solicitar
incriure's como usuario del sistema. Para inscribirse como usuario hace falta enviar un correo electrónico al
\emph{webmaster} encargado de los formularios.

\notavisible{Cambiar en todas partes \emph{registrar} por \emph{inscribir}.}
Para inserir nuevas palabras, se llenará el formulario con los datos correspondientes y se
polsarà el botón \textit{'Go Dónde'}; en este momento hará falta identificarse como usuario
inscrito o no se podrá continuar con el procedimiento de inserir palabras. Cuando
una persona se inscribe se puede escribir de dos maneras: como
\emph{profesional} o como \emph{no profesional}. Cada modalidad tiene unas habilitacions
diferentes, que se explicarán en apartados siguientes.

\subsection{Inserción de entradas}

\subsubsection{Perfil profesional}

Cuando se quiere inserir una nueva entrada a los diccionarios hace falta dirigirse al
...
gramatical de las entradas que se inseriran. Seguidament, se polsa el
botón \textit{Go donde} para seguir con el proceso de inserción.

En este punto aparece una nueva ventana donde muestra los lemas y unos
parámetros que caracterizan la nueva entrada al diccionario. Si la entrada ya
 es en alguno de los diccionarios, se avisará a la persona usuaria y el sistema
automáticamente escogerá sentido unidireccional (de izquierda a derecha o a la
inrevés). Si no es en ninguno, elegirá doble sentido de traducción. 

En esta
nueva ventana se pueden hacer estas tres acciones, si se escau:


\notavisible{Hace falta repasar la primera oración del párrafo siguiente; parece que
hay algún material que tendría que ser borrado; Otra cosa, los
formularios en la actualidad no tienen apoyo para traducciones múltiples,
según parece. Haría falta hacer constar esta circunstancia en algún lugar.}
\begin{itemize}
\item Escoger el paradigma correspondiente para el lema en lengua origen
   y también en lengua meta (esta acción es obligatoria, el resto
  no).footnote/*footnote{Escoger el paradigma es un proceso con el que se tiene que ir
    con mucha cura. Hace falta elegir el paradigma exacto que describe
    el comportamiento del lema que estamos inserint. En
    el caso de adjetivos, nombres y acrònims, hace falta escoger el paradigma que
    se ajuste a la flexió deseada y también a los géneros admitidos. En
    el caso de acrònims hace falta fijarse en el género y número que admeteix
    cada paradigma de los que podemos escoger; por ejemplo BBC sirve
    para acrònims femeninos singulares y SANO para acrònims femeninos que
    admeteixen plural). En el caso de nombres propios hace falta escoger el
    paradigma correspondiente dependiente de si se trata de un nombre propio de cosa
    (un diario), de persona o de lugar.}
\item Escoger la dirección de traducción de la entrada si es diferente de la
  propuesta automáticamente.
\item Añadir comentarios en la entrada que quedarán al diccionario final.
\end{itemize}

Una vez hemos hecho las operaciones correspondientes, polsem el botón
\textit{'Go donde'} si queremos confirmar la entrada o \textit{'Close'} si
sencillamente queremos cancelar la operación de inserción.
...
se muestran en tres ventanas de texto y se pueden editar por si hace falta hacer
alguna modificación. Una vez revisadas las entradas, polsarem el
botón \textit{'Insert'} para inserir-las definitivamente a los
diccionarios correspondientes. También tenemos la posibilidad de polsar el
botón \textit{'Go back'} en caso de que quisimos volver al paso anterior.
...
para el usuario profesional, pero las entradas inserides no se guardan
a los diccionarios generados por los formularios sino que quedan en una cola
de entradas pendientes de validació. Por inserir las palabras de esta
cola a los diccionarios hace falta que un usuario profesional las valide.

\subsection{Validació de entradas pendientes de aprobación}

Los usuarios profesionales, en la pantalla donde se escoge el paradigma
de los lemas inserits, tienen dos enllaços adicionales:
\begin{itemize}
\item \textit{See pairs tono be validated}: Aquí abriremos una ventana donde
  podremos ver el contenido del fichero con las palabras que están
  pendientes de validació; palabras que habrán sido inserides por
  usuarios no profesionales. Esta ventana es meramente informativa y se
  cierra polsant el botón \textit{'Close'}.
\item \textit{Validate pairs}: Esta opción permite a un usuario
  profesional validar una a una las entradas pendientes. Cuando polsem
  este botón, se abre la ventana de selección de paradigmas que se ha
  visto en el apartado de inserción de nuevas palabras. Esta ventana se
  muestra con los datos que había escogido el usuario que había añadido
  esa entrada. Ahora, el usuario profesional podrá modificar los lemas,
...
Este proceso es repite hasta que se validen todas las palabras de la cola o finalizamos el proceso en polsar el botón \textit{'Close'}.

\end {itemize}


\notavisible{FIN DEL TEXTO TRADUCIDO AUTOMÁTICAMENTE, PENDIENTE DE REVISIÓN}
