...
\texttt{dix.dtd} \nota{MG: no caldria ajuntar les dues DTDs en una de sola?} on es defineix l'element \texttt{e} usat per a
...
que es mostren en la pàgina \pageref{entrades_lextor}, on s'explica de quina manera
s'indiquen en el diccionari bilingüe aquests múltiples equivalents.
...
A continuació, en la secció~\ref{se:preprocessament} s'explica el
preprocessament que s'ha de fer d'un diccionari bilingüe que conté més
d'una traducció per a una entrada, tant si s'usa un mòdul de tria
lèxica com si no, i en la secció~\ref{se:lextor} s'explica com
funciona i com s'ha d'entrenar el mòdul que fa la tria lèxica.
...
que permeten l'especificació de més d'una traducció per entrada (vegeu
en la secció \ref{dic_lextor} com s'especifiquen aquestes entrades)
fa necessari un preprocessament d'aquests diccionaris, ja que el motor de
...
Quan els diccionaris bilingües amb múltiples equivalents 
s'utilitzin en un sistema que no disposa de mòdul de selecció lèxica,
el preprocessament es realitza amb l'aplicació del full d'estil
\texttt{translate-to-default\--e\-qui\-va\-lent.xsl}.  Aquest full d'estil
converteix els diccionaris amb més d'una possible traducció per
entrada en diccionaris amb una sola traducció per entrada, escollint
com a traducció l'entrada marcada com a equivalent predeterminat, i
posant una restricció de direcció (\texttt{LR} o \texttt{RL} segons el
cas) en les entrades restants, de manera que aquestes només es
tradueixen en la direcció de traducció en què no hi ha multiplicitat
d'equivalents.  Aquest full d'estil es crida des del
\texttt{Makefile}.

Com a exemple, les tres primeres entrades que es mostren en la pàgina
\pageref{entrades_lextor} queden així després de l'aplicació del full
d'estil esmentat:
...
\nota{Es ``en comptes de'' o ``addicionalment a''?}
la
...
\nota{L'alternança \emph{pseudolema} i \emph{pseudoparaula} s'ha de resoldre}
...
\nota{Comprovar aquest ítem perquè era incomplet i l'ha completat mlf}
...
\notavisible{FIN DEL TEXTO EN CATALÁN, PENIENTE DE TRADUCCIÓN}
