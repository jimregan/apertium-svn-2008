...
\subsection{Datos: especificación del formato de las reglas de transferencia estructural}
\label{formatotransfer}


En esta sección se explica el formato en el que se escriben las reglas
de transferencia estructural. En el apéndice~\ref{ss:dtd_transfer} se
ofrece la descripción formal (DTD).

El archivo de reglas de transferencia estructural tiene dos partes
...
\item Una serie de declaraciones de los \emph{atributos} que queremos
  ...
  la traducción al ser la forma léxica de la LO invariable en género,
...
\item Una sección de \textit{definición de listas de cadenas},
  ...
  ellas un elemento que corresponda para realizar una transformación
  determinada.
...
Es el elemento raíz y contiene todos los demás elementos del archivo de reglas de
transferencia estructural.
...
patrones. Así, si nos interesa detectar todos los nombres para realizar operaciones con ellos, crearemos una
...
todos los adverbios. También puede utilizarse el asterisco para
...
desconocidas se marcan con un asterisco (*); si se desea que no se
marquen las desconocidas, se añade una \texttt{u} al nombre del
formato. Así, las opciones de formato son las siguientes:

\begin{itemize}
\item \texttt{txt} : Opción predeterminada, texto con marca de desconocidas

\item \texttt{txtu} : texto sin marca de desconocidas

\item \texttt{html} : HTML con marca de desconocidas

\item \texttt{htmlu} : HTML sin marca de desconocidas

\item \texttt{rtf} : RTF con marca de desconocidas

\item \texttt{rtfu} : RTF sin marca de desconocidas 
...
Como todos los datos procesados por el sistema, desde el texto
original al texto traducido, circulan entre los ocho módulos del
sistema en formato de texto, es posible detener el flujo de texto en
cualquier punto para examinar la entrada o la salida de un módulo
determinado. Para ello sólo hay que escribir los comandos adecuados en
el terminal utilizando una estructura de tubería, de modo que la
salida estándar de un módulo se utilice como la entrada estándar del
siguiente.

Mediante los comandos \texttt{echo} o \texttt{cat} se puede enviar texto a través de uno o varios módulos para analizar
la salida y detectar el origen del error. Debe irse al directorio en que se guardan los datos lingüísticos
y escribir los comandos explicados a continuación.

\subsection{La salida del analizador morfológico}
...
\subsection{La salida del desambiguador}
...
\begin{small}
\begin{alltt}
echo "sabates" | apertium-destxt | lt-proc ca-es.automorf.bin \\|./ca-es.tagger --tagger ca-es
\end{alltt}
\end{small}
...
\subsection{La salida del módulo \texttt{pretransfer}}
...
\begin{small}
\begin{alltt}
echo "sabates" | apertium-destxt | lt-proc ca-es.automorf.bin \\|./ca-es.tagger --tagger ca-es | apertium-pretransfer
\end{alltt}
\end{small}

Como \emph{sabates} no es una multipalabra este módulo no altera la entrada.

\subsection{La salida del módulo de transferencia estructural}
...
\begin{small}
\begin{alltt}
echo "sabates" | apertium-destxt | lt-proc ca-es.automorf.bin \\|./ca-es.tagger --tagger ca-es | apertium-pretransfer \\| ./ca-es.transfer ca-es.autobil.bin
\end{alltt}
\end{small}
...
\subsection{La salida del generador morfológico}

Para saber cómo el sistema genera una palabra, escriba lo siguiente en el terminal:

\begin{small}
\begin{alltt}
echo "sabates" | apertium-destxt | lt-proc ca-es.automorf.bin \\|./ca-es.tagger --tagger ca-es | apertium-pretransfer \\| ./ca-es.transfer ca-es.autobil.bin | ltproc -g ca-es.autogen.bin
\end{alltt}
\end{small}
...
\subsection{La salida del postgenerador}
...
\begin{small}
\begin{alltt}
echo "sabates" | apertium-destxt | lt-proc ca-es.automorf.bin \\|./ca-es.tagger --tagger ca-es | apertium-pretransfer \\| ./ca-es.transfer ca-es.autobil.bin | ltproc -g ca-es.autogen.bin \\| ltproc -p es-ca.autopgen.bin
\end{alltt}
\end{small}

\subsection{La salida de Apertium}
...
\begin{small}
\begin{alltt}
echo "sabates" | apertium-destxt | lt-proc ca-es.automorf.bin \\|./ca-es.tagger --tagger ca-es | apertium-pretransfer \\| ./ca-es.transfer ca-es.autobil.bin | ltproc -g ca-es.autogen.bin \\| ltproc -p es-ca.autopgen.bin | apertium-retxt
\end{alltt}
\end{small}
...
Cuando se produce algún tipo de error lingüístico en la traducción, el sistema da como salida un símbolo seguido
de la forma léxica causante del problema. El significado de los símbolos es el siguiente:

\begin{itemize}

\item \verb!#! = El problema está en el generador, que no puede generar la forma léxica (el diccionario morfológico no la contiene en la dirección de generación)

\item \verb!@! = El problema está en el módulo de transferencia léxica, que no puede traducir la forma léxica (el 
diccionario bilingüe no la contiene)
...
(femenino), con lo qual provoca un error del traductor:
...
de datos lingüísticos, debe escribirse \texttt{make
  tagger\_supervised} o \texttt{make tagger\_unsupervised} según sea
el caso.
