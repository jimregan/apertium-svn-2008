...
añadido un atributo a los diccionarios bilingües, que se describe en la página 
\pageref{dic_lextor}.
...
respectivamente). En la página \pageref{dic_lextor}, además, 
se describen las características de un diccionario bilingüe que funcione en un 
sistema Apertium que disponga de módulo de selección léxica.
...
\paragraph{Diccionario bilingüe para un sitema con selección léxica}\label{dic_lextor}
...
A l'Apèndix~\ref{dixdtd} es mostra el fragment de la DTD
...
aquesta és la traducció predeterminada, és a dir, la traducció a emprar
...
\notavisible{FIN DEL TEXTO EN CATALÁN}
...
\section{Mòdul de selecció lèxica}
...
secció~\ref{dic_lextor} que hauria d'ubicar-se a la secció 3.1.2
(pàgina 26) on s'expliquen els diccionaris usats per Apertium.}

\subsection{Introducció} pingui1
...
Abans d'explicar-ne les característiques, veurem com els 
problemes d'\emph{equivalència
múltiple} (l'existència de
més d'una traducció possible en llengua meta per a un lema o una forma 
lèxica en llengua
...
(amb els atributs \texttt{LR} o \texttt{RL}) per tal que siguin traduïts en la direcció
...
d'una manera o de l'altra, de vegades no és senzill. Per exemple, 
quan entre dos o
...
codificades amb els atributs \texttt{slr} o \texttt{srl},
...
a entrada predeterminada) queda com a traducció fixa del lema en llengua origen.
...
\texttt{es-ca} \emph{hoja -- full/fulla} o \emph{muñeca -- nina/canell}, i els exemples \texttt{en-ca}
mostrats en el següent apartat. (Mireia: ara no hi són pingui!)
...
secció~\ref{dic_lextor} explica la modificació feta per a
...
fer del diccionari bilingüe, tant si s'usa un mòdul 
de tria lèxica com si no. Finalment en la secció~\ref{se:lextor} s'explica
com funciona i com s'ha d'entrenar el mòdul que fa la tria lèxica.



\subsection{Preprocessament dels diccionaris bilingües}\label{se:preprocessament}
...
\subsubsection{Preprocessament sense mòdul de tria lèxica}
...
l'aplicació del full d'estil \texttt{translate-to-default-equivalent.xsl}.  Aquest full
...
traducció l'entrada marcada com a equivalent predeterminat, i posant una
...
Com a exemple, les tres primeres entrades que es mostren en la pàgina
\ref{dic_lextor} queden així després de l'aplicació del full
d'estil esmentat:
...
\subsubsection{Preprocessament amb mòdul de tria lèxica}
...
\subsection{Funcionament del mòdul per a la tria lèxica}\label{se:lextor}
...
\subsubsection{Entrenament}\label{se:entrenament}
...
\subsubsection{Utilització}\label{se:us}
...
\item \textbf{Uso en el módulo interchunk}
  ...
  \texttt{<\textbf{clip}>})
\notavisible{MG: generalmente o sempre}, el lema del chunk (\texttt{lem}), las
...
\subsection{Características de los tres módulos de transferencia}

Como ya se ha explicado, el sistema de transferencia sintática avanzada o 
Apertium 2 funciona con tres módulos, cuyas características diferenciadas
se describen a continuación. Cuando Apertium funcione con un sistema
