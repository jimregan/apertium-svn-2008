...
el programa \texttt{chunker}, però ha de quedar clar que s'utilitza un arxiu
diferent. Els segments són tractats de forma similar a com es
tracten les paraules en el transfer o \texttt{chunker} d'Apertium.
\notavisible{Comprovar la denominació dels programes}

\paragraph{Entrada/eixida}

\begin{itemize}
\item Entrada: segments procedents de \texttt{chunker}.
...
\texttt{chunker}, i s'especifica en l'apèndix
...
\texttt{chunker} per a realitzar la detecció.
...
\texttt{chunker}--- són les següents:
...
\item A diferencia del que succeeix en el mòdul \texttt{chunker}, no
  es permet d'imprimir una altra cosa que \texttt{<chunk>}s en les
  instruccions \texttt{<out>} en el fitxer d'especificació del
  comportament d'aquest mòdul, i en cap cas paraules soltes.
\end{itemize}
...
implícita que escriu sense modificar els segments no reconeguts per cap
...
\item Eixida: entrada vàlida per al mòdul de generació morfològica d'Apertium.
\end{itemize}
...
\texttt{chunker} i en l'\texttt{in\-ter\-chunk} d'Apertium 2. 
...
\texttt{in\-ter\-chunk} són les següents:
...
\item Tampoc existeix l'atribut \texttt{side} als elements als quals
  és aplicable en el mòdul \texttt{in\-ter\-chunk}, per
  la mateixa raó que en \texttt{in\-ter\-chunk} no
  s'accedeix al diccionari bilingüe.
\notavisible{MG: però llavors això no és una diferència respecte de \texttt{interchunk} no?}
\end{itemize}
...
mateix ordre en què són llegits, ja que la detecció de segments és
...
\notavisible{MG: abans deia 'al nom de la regla', comentari mlf: De la
...
\notavisible{Potser s'hauria d'explicar per què s'ha triat el nom \emph{a\_nom} en la figura}
...
de la concatenación de un \texttt{clip} (el lema de la posición 3) y una etiqueta \emph{adv}.}
\label{fig:concat}
\end{figure}
