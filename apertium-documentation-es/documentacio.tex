...
\nota{Mikel: pots revisar la introducció?}
...
%Barra invertida & \verb!\! & \verb!\\! & Carácter de protección \\Mireia treure
...
\item \textit{Una definición del alfabeto}.  Esta definición se usa
  ...
  las de las secciones condicionales (ver \ref{ss:section} de forma adecuada; el generador
  morfológico no necesita esta definición.
...
\item \textit{Una o más secciones de diccionario de recorte
  ...
  sigue (ver discusión en el apartado \ref{ss:section}). En los
  diccionarios morfológicos de catalán, las palabras que requieren un
  recorte incondicional se reparten en dos secciones: una para las
  formas que necesitan que se coloque un blanco a continuación (por
  razones de procesamiento de las formas léxicas), como \emph{l'} o
  \emph{d'}, y otra para signos de puntuación, números y otros signos.
...
se da en el apéndice~\ref{ss:dtd_dics}. En las páginas
\pageref{ss:morfgen}, \pageref{ss:bil} y \pageref{ss:postgen} se describen las
características particulares de las entradas de los tres tipos de
diccionarios (morfológicos, bilingües y de postgeneración
respectivamente).
...
A partir de 2007, Apertium funciona con dos sistemas de transferencia
estructural diferentes, que denominamos Apertium 1 y Apertium 2.

Apertium 1 es el sistema inicial, que realiza las transformaciones
estructurales en una única pasada de las reglas, las cuales detectan
secuencias o \emph{patrones} de formas léxicas y realizan sobre ellas
las comprobacions y cambios pertinentes.  

Apertium 2 es un sistema más avanzado, que funciona con una nueva
arquitectura de transferencia que permite detectar \emph{patrones de
  patrones} de formas léxicas.  Su necesidad aparece con el
...
momento, como por ejemplo el par \emph{inglés}--\emph{catalán}. Este
sistema de transferencia se compone de tres módulos, el primero de los
cuales puede identificarse con el módulo de transferencia de Apertium
1.0, con ligeras variaciones.

A continuación se describen sus características.  En el apartado
\ref{functransfer} se describe el sistema Apertium 1.0, y en el 
apartado \ref{apertium2}, el sistema Apertium 2.0. Puesto que,
como hemos dicho, éste es una ampliación del primer sistema, la
descripción general de Apertium 1.0 es válida también para Apertium 2.0,
con las diferencias que se exponen en el apartado dedicado al mismo.
Finalmente, en el apartado \ref{formatotransfer} se describe el formato
utilizado para crear reglas en ambos sistemas.


\subsection{Descripción del funcionamiento: Apertium 1}
\label{functransfer}
...
\subsection{Descripción del funcionamiento: Apertium 2}
\label{apertium2}

La arquitectura de transferencia de Apertium 1 descrita en el apartado
...
definidas por el usuario. Este modelo, que denominamos de
\textit{transferencia sintàctica superficial}, considera dos niveles desde
...
detectado. Todo este procesamiento de detección y manipulación lèxica
y de patrones se lleva a cabo en una única pasada.  

Como contraste, la
nueva arquitectura de transferencia de Apertium 2 se
define como un sistema de transferencia lèxica adelantada en tres
niveles y tres pasadas. Los dos primeros niveles, el nivel léxico y el
de patrones, son homólogos a los del transfer de Apertium. El nuevo
...
en los patrones de palabras de Apertium. Con esta nueva estructura de
...
de patrones de palabras en Apertium 1 no tiene que coincidir con la
identificación de patrones en Apertium 2: se pretende que los patrones
en Apertium 2 tengan un \textit{espíritu} de sintagmas que no tenían
en Apertium. Por esta razón utilizaremos el término \textit{segmento}
(en inglés \textit{chunk}), para referirnos a las secuencias de
palabras de Apertium 2. 

La arquitectura de transferencia de Apertium 2
...
\begin{itemize} 
\item \texttt{transfer} (intrachunk): identifica los segmentos,
  ...
  concordancia). Además, crea los segmentos para que sean tratados por el
  módulo siguiente.
\item \texttt{interchunk}: este módulo recibe los segmentos
  construidos por el \texttt{in\-tra\-chunk} y permite reordenarlos,
...
La especificación concreta de las tareas de cada módulo se encuentra en las secciones
correspondientes.
...
\emph{intrachunk} i \emph{transfer}; caldria elegir una i usar-la en tot el
document.}


La comunicación entre los módulos \texttt{intrachunk} y
...
\item Si la primera letra del pseudolema está en mayúsculas y el resto
  ...
  \texttt{trans\-fer\--post}, se hará que la primera letra que
...
y se hace mediante una macro o
algún mecanismo similar.

\subsubsection{Programa \texttt{transfer} (\texttt{intrachunk})}

\notavisible{ATENCIÓ: TEXT EN CATALÀ, PENDENT DE TRADUCCIÓ}
...
\paragraph{Entrada/eixida}

\begin{itemize}
\item Entrada: dades amb el format d'eixida del \texttt{pretransfer}, és a dir,
...
manera idèntica a Apertium. El nom del fitxer de patrons tindrà 
...
és necessari per a adaptar el format
...
\paragraph{Especificació de les accions associades}
\label{ass_intra}


És bàsicament similar a l'actual mòdul de transferència d'Apertium,
amb les següents incorporacions:

\begin{itemize}
\item Especificació de l'acció implícita en l'element arrel
  ...
  funcione a la manera d'Apertium 1 o d'Apertium 2, respectivament.
...
\item Generació de segments (chunks): l'etiqueta \texttt{<chunk>} és una etiqueta d'un nivell més alt
  que \texttt{<lu>} (\textit{lexical unit}) que
  permet generar segments amb les característiques que han sigut
  especificades a la secció anterior; \texttt{<chunk>} i té els següents atributs:
  \begin{itemize}
...
\item Les instruccions \texttt{<clip>} tenen un nou atribut opcional:
  ...
  \verb!<!\textit{valor de link-to}\verb!>! en comptes de 
...
\item \texttt{chunk}: en el cas que es vulga que el mòdul genere
  ...
  és el cas d'Apertium 2. El nou segment es crearà
  mit\-jan\-çant la traducció de la paraula pel diccionari bilingüe.
  El lema fix dels chunks creats implícitament és
  \texttt{default}.
\item \texttt{lu} (valor implícit): el funcionament actual per
  defecte del mòdul de transferència d'Apertium 1, traduint la paraula
  amb el diccionari bilingüe.
\end{itemize}

A continuació es mostra el resultat del processament implícit d'un
...
el programa \texttt{transfer} d'Apertium 1 o el \texttt{intrachunk}
...
\texttt{.t2x} pera analogia amb el mòdul anterior, i un
...
\item A diferencia del que succeeix en el \texttt{trans\-fer\--in\-ter\-chunk},
\notavisible{Denominació!!} no es
  permet d'imprimir una altra cosa que \texttt{<chunk>}s en les instruccions
...
Al igual que en el mòdul anterior, s'hi ha establert una acció
implícita que escriu sense modificar els segments no reconegut per cap
patró del fitxer d'especificació d'aquest mòdul. Aquesta acció implícita
escriu exactament el que llig, tant si es tracta de chunks com
de blancs.
...
cadascun d'ells, executa unes accions especificades. La detecció es
basa en els lemes del segment, 
\notavisible{Seria \emph{en el lema}?}
i no en patrons (no en les etiquetes);
això farà que la detecció en aquest mòdul siga específica per a cada
``nom'' de segment.\notavisible{Quan fixem bé la terminologia hem d'assegurar-nos que la redacció d'aquesta part és l'adequada.}

D'altra banda, el funcionament de la detecció i del processament de
les regles es basa en que les referències a paràmetres es resolen
immediatament després de la detecció, és a dir, les etiquetes
\texttt{<1>}, \texttt{<2>}, etc. són substituïdes automàticament pel
valor dels paràmetres abans de començar el processament. Les posicions
(atribut \texttt{pos}) a les quals es fa referència amb les
instruccions \texttt{<clip>}, etc. corresponen a la posició de les
paraules en l'interior del segment.

També aplica automàticament la política de majúscules (veure secció
...
La detecció dels segments es basa en el nom dels segments. Els
segments no detectats reben el processament implícit.
...
\begin{itemize}
\item Prohibició de l'escriptura de segments (\texttt{<chunk>}s) a
  l'eixida: només es poden escriure unitats lèxiques d'un mot
  (\texttt{<lu>}) o de més d'un mot (\texttt{<mlu>}).
\notavisible{Comprovar aquest ítem perquè era incomplet i l'ha completat mlf}
\item Nou atribut de detecció \texttt{name} de \texttt{<pattern-item>},
  que s'usa en la part de \texttt{<pattern>} de les regles de manera
  aïllada, per tal de forçar la detecció dels patrons pel seu nom.
...
En aquest mòdul l'acció implícita és la d'escriure les paraules de
...
que aquest mòdul s'utilitze per a casos molts específics en què
calga fer algun processament molt específic.

També s'aplica implícitament la política de majúscules (secció
\ref{ss:majuscules}).

En qualsevol cas, els blancs externs als segments són copiats en el
mateix ordre en el que són llegits, ja que la detecció de segments és
unitària (aquest mòdul no els agrupa).

\notavisible{FI TEXT EN CATALÀ, PENDENT DE TRADUCCIÓ}
...
(pingui treure! ~\ref{ss:dtd_transfer}) se
...
(\textit{Apertium 2: Sólo en transfer/intrachunk})
\notavisible{Decidim-nos per un nom}


Es el elemento raíz (en Apertium 1) o el elemento raíz del
módulo transfer/intrachunk (en Apertium 2) y contiene todos los demás
...
\item \texttt{lu}: indica que funcionará como un 
transfer de Apertium 1 y, por lo tanto, que las palabras implícitas deben dejarse intactas
...
esta palabra para que pueda ser tratada por los siguientes módulos en Apertium 2.
...
(\textit{Sólo en interchunk de Apertium 2})

Es el elemento raíz del módulo interchunk y contiene todos los demás
...
(\textit{Sólo en postchunk de Apertium 2})

Es el elemento raíz del módulo postchunk y contiene todos los demás
elementos del archivo de reglas de transferencia
estructural de este módulo.
...
Este elemento tiene dos usos diferenciados según el sistema de transferencia y el
módulo en el que se utilice.

\paragraph{Uso en Apertium 1 y en el módulo transfer/intrachunk de Apertium 2}
...
Cada elemento \texttt{<\textbf{cat-item}>} tiene un atributo obligatorio
\texttt{tags} cuyo valor es una secuencia de símbolos gramaticales
separados por puntos; esta secuencia es una subsecuencia de la
etiqueta fina, es decir, de la secuencia de símbolos gramaticales que
define cada posible forma léxica entregada por el desambiguador léxico
categorial.  De este modo, una categoría representa un determinado
conjunto de formas léxicas.  Tenemos que definir tantas categorías
distintas como tipos de formas léxicas queramos detectar en los
patrones. Así, si nos interesa detectar todos los nombres (en Apertium 1 o en
el \texttt{intrachunk}) para realizar operaciones con ellos, crearemos una
categoría definida a partir del símbolo gramatical \texttt{n}. Por otro lado, si nos interesa detectar
todos los nombres femeninos en plural, deberemos definir una categoría mediante los símbolos 
...
\paragraph{Uso en el módulo interchunk de Apertium 2}


Se utiliza igual que en el módulo transfer/intrachunk, pero aquí, en lugar de definirse
a partir de los símbolos gramaticales de las formas léxicas, se define a partir de los símbolos de
los segmentos entregados por el módulo transfer/intrachunk. Si, por ejemplo, queremos definir
...
\ref{{ass_intra}}). También se puede utilizar el atributo opcional
\texttt{lemma} para hacer referencia al \emph{pseudolema} del segmento. Así pues, sus características
formales son las mismas en el módulo transfer y en el interchunk, con la única diferencia que en el
primero se utilizan para detectar formas léxicas y el en segundo, para detectar segmentos.


\paragraph{Uso en el módulo postchunk de Apertium 2}

Este elemento solo prevé el atributo obligatorio \texttt{name}, que hace referencia al nombre del
segmento o chunk,

\notavisible{MG: abans deia 'al nom de la regla', comentari mlf: De la regla o del patró?}
sin etiquetas, ja que en el módulo \texttt{postchunk}
sólo se usa el pseudolema (nombre del segmento) para la detección. 
En la detección no se distingue entre mayúsculas y minúsculas, porque el pseudolema se usa para transmitir la información sobre las mayúsculas y las minúsculas del segmento. (Véase la figura \ref{fig:cat-item-postchunk}).
...
morfológica (tanto información flexiva --género, nombre, persona,
...
\emph{número} y el atributo de \emph{nombre}. Compárese la definición del
...
\caption{Definición del atributo de categoría \emph{número}
 ...
 \emph{número por determinar}, así como del atributo de categoría \emph{nombre}, que puede
...
En esta sección se definen (mediante \texttt{<\textbf{def-var}>}) las
...
reglas.  Las variables transmiten cadenas que describen información de estado, como la
...
En esta sección se definen (mediante \texttt{<\textbf{def-list}>}) las
...
\begin{small}
\begin{alltt}
<\textbf{def-list} n="verbos_estado">
...
Cada regla tiene un patrón (\texttt{<\textbf{pattern}>}) y la acción (\texttt{<\textbf{action}>}) asociada a él que se ejecuta cuando es detectado.
...
mayúsculas y minúsculas de una forma léxica) y
\texttt{<\textbf{call-macro}>} (llamadas a macroinstrucciones).
...
realizadas dentro un patrón, se asignan valores a atributos del mismo
y, en su caso, a variables globales o de estado y se envía la
información del patrón en LM resultante al siguiente módulo (el generador morfológico).
...
ello, hay que especificar su nombre en el atributo \texttt{n} y uno o
...
Con este elemento se describe una opción condicional (véase
la página \pageref{choose}).  Se compone de la condición a verificar
\texttt{<\textbf{test}>} y de un bloque de cero o más instrucciones de
tipo \texttt{<\textbf{choose}>}, \texttt{<\textbf{let}>},
\texttt{<\textbf{out}>}, \texttt{<\textbf{modify-case}>} y
\texttt{<\textbf{call-macro}>}, que se ejecutarán si se cumple la
citada condición.
...
\texttt{<\textbf{let}>}, \texttt{<\textbf{out}>}, \texttt{<\textbf{modify-case}>} y
\texttt{<\textbf{call-macro}>}) que deben realizarse si no se cumple
...
condiciones a evaluar, así como por una simple condición de igualdad
(\texttt{<\textbf{equal}>}).
...
\item El elemento de negación \texttt{<\textbf{not}>} representa una condición
  que se cumple cuando la condición incluida no se cumple y viceversa. Se puede ver un ejemplo de negación de una igualdad en la figura \ref{fig:ornot}.

\item El operador condicional más simple es el de igualdad
...
\item El operador de búsqueda en listas, \texttt{<\textbf{in}>}, que
  sirve para buscar cualquier valor (que corresponde al primer
  parámetro de la condición en una lista definida en la sección
  correspondiente (\texttt{<\textbf{section-\\def-lists}}), a la que
  se hace referencia mediante el atributo \texttt{n} del elemento \texttt{<\textbf{list}>}.
  La búsqueda es cierta si el valor se encuentra en la lista.  En esta
  comparación se puede utilizar también el atributo \texttt{caseless}:
  si se le asigna el valor \texttt{yes}, se realiza la búsqueda en la
  lista sin tener en cuenta las diferencias de mayúsculas y
  minúsculas. En la figura \ref{fig:in} puede verse un ejemplo de su
  uso.
...
forma léxica de la lengua de origen o de la lengua de destino,
definida por el valor de varios atributos (véase un ejemplo de uso en
la figura \ref{fig:clip}):

\begin{itemize}
\item \texttt{pos} es un índice (1, 2, 3, etc.) utilizado para seleccionar una
  forma léxica dentro de la regla: se corresponde con el lugar que
  ocupa la forma léxica en el patrón.

\item \texttt{side} especifica si se selecciona un \emph{clip} de la lengua de
  origen (\texttt{sl}, por \emph{source language}) o de la lengua de destino
  (\texttt{tl}, por \emph{target language}).
  
\item en \texttt{part} se indica qué parte de la forma léxica se
  procesa; generalmente su valor es uno de los atributos definidos en
  ...
  símbolos gramaticales, con sus valores actuales en el caso de que
  hayan sido modificados en la parte precedente de la regla).
...
\end{alltt}
\end{small}
\caption{Fragmento de una regla en la que se comprueba si el género en LO de la primera o de la segunda unidad léxica identificada en un patrón (podría ser determinante--adjetivo--nombre) es distinto del género de la tercera unidad léxica también en LO.}
\label{fig:ornot}
\end{figure}
...
\texttt{<\textbf{let}>} o en un \texttt{<\textbf{modify-case}>}, representa la referencia de la variable y se
        ...
        <\textbf{when}>
          <\textbf{test}>
            <\textbf{in} \textsl{caseless}="yes"/>
              ...
              <\textbf{list} \textsl{n}="verbos_estado"/>
            <\textbf{/in}>
         <\textbf{/test}>
         <\textbf{let}>
...
\subsubsection{Elemento de información de mayúsculas/minúsculas\\  \texttt{<get-case-from>}}
...
\subsubsection{Elemento de obtención del patrón de mayúsculas/minúsculas\\ \texttt{<case-of>}}
...
\texttt{tl} para la LM, y \texttt{part}, el atributo concreto al que
se hace referencia (normalmente el lema), con los atributos
predefinidos que ya se mencionan en la página \pageref{ss:clip}. En la figura
...
valores que toma este valor suelen ser ``\texttt{Aa}' para expresar
que la ``parte izquierda'' de esta modificación de las mayúsculas debe
tener la primera letra en mayúsculas y el resto en minúsculas,
``\texttt{aa}'' para poner todo en minúsculas y ``\texttt{AA}'' para
poner todo en mayúsculas. En la figura \ref{fig:case} puede verse un
ejemplo de su utilización. En esta regla se modifica el estado de
mayúsculas del lema en LM en posición "1", que corresponde a
\emph{dir}, puesto que, aunque a la salida de la regla sea la segunda
forma léxica (\emph{vaig dir}), es la traducción de la FL que tiene la
posición 1 en la LO, y por lo tanto sigue teniendo asignada esta
posición en la LM. A este lema se le asigna el valor ``\texttt{aa}''
en caso de que el lema en LO tenga el estado ``\texttt{Aa}''. En los
demás casos no hay que especificar nada, pues el estado de
mayúsculas/minúsculas en la FL con posición 1 coincidirá en la LO y en
la LM y, por lo tanto, se transfiere automáticamente (véase la
página~\pageref{mayusc} para más información sobre la gestión de
mayúsculas y minúsculas en los diccionarios).
...
\end{alltt}
\end{small}
\caption{Regla para \texttt{es}--\texttt{ca} que transforma los verbos
...
operaciones de transferencia estructural pertinentes. La instrucción
envía cada forma léxica dentro de un conjunto \texttt{<\textbf{lu}>}, que a su vez
puede estar contenido dentro de  un elemento \texttt{<\textbf{mlu}>}  si lo que
se envía es una multipalabra compuesta de dos o más FL. Además, 
se envían también los espacios en blanco o superblancos (\texttt{<\textbf{b}>}) que haya que 
colocar entre FL y FL.
 

En las figuras \ref{fig:case} y \ref{fig:regla2} puede verse
un ejemplo de uso.
...
atributo \texttt{whole} puede utilizarse para enviar cualquier tipo de forma léxica
excepto los verbos, puesto que sólo hemos definido multipalabras con flexión para formas verbales.
...
ficheros de definición se encuentran en el directorio de datos
...
$L_1$\texttt{-tagger-data} y $L_1$\texttt{-tagger-data} que están
contenidos en el directorio anterior. En este directorio se encuentran
los ficheros siguientes (ejemplos de \texttt{es-tagger-data}):

\begin{itemize}

\item \texttt{es.tagged.txt}: Corpus españo en formato de texto llano.
\item \texttt{es.untagged}: Corpus analizado morfológicamente
  (procesado por el desformateador de texto y el analizador morfológico).
...
\item \texttt{es.crp.txt}: Un corpus grande (cientos de miles de
  palabras) utilizado cuando se entrena el desambiguador de forma sno
  supervisada mediante el algoritmo de maximización de la esperanza
  matemática.
\item \texttt{es.crp}: El corpus anterior procesado consecutivamente por el desformateador de texto y el analizador morfológico.
\item \texttt{es.dic}: Calculado a partir del diccionario
  \texttt{*.es.dix}, y mediante el uso de las herramientas
  \texttt{lt-expand} y \texttt{apertium-filter-ambiguity} contiene
  todas las formas distintas que el etiquetador definido mediante el
  archivo \texttt{*.es.tsx} identifica como clases de ambigüedad
  diferentes (es decir, qué categorías léxicas pueden ser homógrafas).
  Si se añaden al diccionario palabras que comporten la aparición de
  una nueva clase de ambigüedad, será necesario volver a generar este
  fichero.

\end{itemize}

Al bajarse el traductor Apertium de Sourceforge
(\url{http://apertium.sourceforge.net/} se obtendrán sólo los tres
primeros ficheros, de modo que deberán generarse los últimos tres,
expandiendo los diccionarios y creando un corpus de gran tamaño,
aunque este gran corpus no es necesario si no se va a entrenar el
desambiguador con reestimación de Baum-Welch. 

Rl entrenamiento del desambiguador puede ser supervisado (con texto
desambiguado a mano) o no supervisado (sin texto desambiguado a mano), de modo que:

\begin{itemize}
\item Para entrenar el desambiguador utilizando texto desambiguado a
  mano se necesitan los ficheros (para español): \texttt{es.untagged},
  \texttt{es.tagged}, \texttt{es.dic}.

\item Para entrenar el desambiguador de forma no supervisada mediante
  el algoritmo de Baum y Welch se necesitan los ficheros (para
  español): \texttt{es.dic}, \texttt{es.crp}
\end{itemize}

Los ficheros \texttt{*.crp}, \texttt{*.untagged}, \texttt{*.dic} son
generados automáticamente mediante \texttt{make} a partir del resto de
los ficheros.

 Aparte de estos ficheros
contenidos en los directorios $L_1$\texttt{-tagger-data} y
$L_1$\texttt{-tagger-data}, se necesitan los mencionados ficheros de
definición
\texttt{apertium-}$L_1$\texttt{-}$L_2$\texttt{.}$L_1$\texttt{.tsx} y
\texttt{apertium-}$L_1$\texttt{-}$L_2$\texttt{.}$L_2$\texttt{.tsx}.
Este fichero contiene la definición de las etiquetas gruesas (o
...
De acuerdo con todo lo expuesto, se pueden modificar los datos del
desambiguador de dos maneras:
...
\item Modificar los corpus utilizados para entrenar el desambiguador.
  ...
  puesto que es posible que la mala desambiguación de ciertas palabras
  se deba a su inexistencia en los corpus de entrenamiento.

  Si se modifican los datos utilizados por el desambiguador (los
  corpus o el fichero de definición) es necesario reentrenarlo; para
  ello, debe escribirse \texttt{make tagger\_supervised} o
  \texttt{make tagger\_unsupervised} en el directorio en el que se
  guardan los datos lingüísticos, dependiendo de si el entrenamiento
  es supervisado o no supervisado, respectivamente.
...
<!\textsl{ATTLIST} pardef
        n \textsl{CDATA #REQUIRED}>

<!\textsl{ELEMENT} \textbf{section} (e+)>
<!\textsl{ATTLIST} section
        id \textsl{ID #REQUIRED}
        type (standard|inconditional) \textsl{#REQUIRED}>
...
<!\textsl{ATTLIST} g
        i \textsl{CDATA #IMPLIED}>

<!\textsl{ELEMENT} \textbf{j} \textsl{EMPTY}>

<!\textsl{ELEMENT} \textbf{s} \textsl{EMPTY}>
<!\textsl{ATTLIST} s
        n \textsl{IDREF #REQUIRED}>
\end{alltt}
\end{small}
...
\section{DTD del módulo de transferencia estructural}
\label{ss:dtd_transfer}

DTD para el formato de las reglas de transferencia estructural.  Esta
Esta definición viene con el paquete \texttt{apertium} (última
versión) que se puede descargar de \url{http://www.sourceforge.net}.
...
\begin{small}
\begin{alltt}
<!\textsl{ENTITY} \% condition "(and|or|not|equal|in)">

<!\textsl{ENTITY} \% container "(var|clip)">

<!\textsl{ENTITY} \% sentence "(let|out|choose|modify-case|call-macro)">

<!\textsl{ENTITY} \% value "(b|clip|lit|lit-tag|var|get-case-from|case-of)">

<!\textsl{ENTITY} \% stringvalue "(clip|lit|var|get-case-from|case-of)">

<!\textsl{ELEMENT} \textbf{transfer} (section-def-cats, section-def-attrs, 
...
<!\textsl{ATTLIST} def-cat  n \textsl{ID} \textsl{#REQUIRED}>

<!\textsl{ELEMENT} \textbf{cat-item} \textsl{EMPTY}>
<!\textsl{ATTLIST} cat-item lemma \textsl{CDATA} \textsl{#IMPLIED} 
                   tags \textsl{CDATA} \textsl{#REQUIRED} >

<!\textsl{ELEMENT} \textbf{section-def-attrs} (def-attr+)>

<!\textsl{ELEMENT} \textbf{def-attr} (attr-item+)>
<!\textsl{ATTLIST} def-attr n \textsl{ID} \textsl{#REQUIRED}>

<!\textsl{ELEMENT} \textbf{attr-item} \textsl{EMPTY}>
<!\textsl{ATTLIST} attr-item tags \textsl{CDATA} \textsl{#IMPLIED}>

<!\textsl{ELEMENT} \textbf{section-def-vars} (def-var+)>

<!\textsl{ELEMENT} \textbf{def-var} \textsl{EMPTY}>
<!\textsl{ATTLIST} def-var n \textsl{ID} \textsl{#REQUIRED}>

<!\textsl{ELEMENT} \textbf{section-def-lists} (def-list)+>

<!\textsl{ELEMENT} \textbf{def-list} (list-item+)>
<!\textsl{ATTLIST} def-list n \textsl{ID} \textsl{#REQUIRED}>

<!\textsl{ELEMENT} \textbf{list-item} \textsl{EMPTY}>
<!\textsl{ATTLIST} list-item v \textsl{CDATA} \textsl{#REQUIRED}>

<!\textsl{ELEMENT} \textbf{section-def-macros} (def-macro)+>

<!\textsl{ELEMENT} \textbf{def-macro} (\%sentence;)+>
<!\textsl{ATTLIST} def-macro n \textsl{ID} \textsl{#REQUIRED}
                    npar \textsl{CDATA} \textsl{#REQUIRED}>
...
<!\textsl{ELEMENT} \textbf{out} (mlu|lu|b)+>
...
<!\textsl{ATTLIST} with-param pos \textsl{CDATA} \textsl{#REQUIRED}>

<!\textsl{ELEMENT} \textbf{clip} \textsl{EMPTY}>
<!\textsl{ATTLIST} clip pos \textsl{CDATA} \textsl{#REQUIRED}
               side (sl|tl) \textsl{#REQUIRED}
               part \textsl{CDATA} \textsl{#REQUIRED}>

<!\textsl{ELEMENT} \textbf{lit} \textsl{EMPTY}>
<!\textsl{ATTLIST} lit v \textsl{CDATA} \textsl{#REQUIRED}>

<!\textsl{ELEMENT} \textbf{lit-tag} \textsl{EMPTY}>
<!\textsl{ATTLIST} lit-tag v \textsl{CDATA} \textsl{#REQUIRED}>
...
<!\textsl{ATTLIST} get-case-from pos \textsl{CDATA} \textsl{#REQUIRED}>

<!\textsl{ELEMENT} \textbf{case-of} \textsl{EMPTY}>
<!\textsl{ATTLIST} case-of pos \textsl{CDATA} \textsl{#REQUIRED}
                  side (sl|tl) \textsl{#REQUIRED}
                  part \textsl{CDATA} \textsl{#REQUIRED}>
...
<!\textsl{ELEMENT} \textbf{b} \textsl{EMPTY}>
<!\textsl{ATTLIST} b pos \textsl{CDATA} \textsl{#IMPLIED}>
\end{alltt}
\end{small}
