...
string const PatternList::ANY_CHAR  = "<ANY_CHAR>";
string const PatternList::ANY_TAG   = "<ANY_TAG>";
string const PatternList::QUEUE = "<QUEUE>";
...
void
PatternList::insertOutOfSequence(string const &lemma, string const &tags,
				 vector<int> &result)
{
  if(lemma == "")
  {
...
void
PatternList::insertIntoSequence(int const id, string const &lemma, 
				string const &tags)
{
  sequence_id = id;
...
void
PatternList::insert(int const id, string const &lemma, string const &tags)
{
  if(!sequence)
  {
    vector<int> local;
...
int
PatternList::tagCount(string const &tags)
{
  int count = 0;
...
string
PatternList::tagAt(string const &tags, int const index)
{
  int start = 0;
  int end = 0;
  int count = 0;
...
void
PatternList::print() const
{
  for(PatternStore::const_iterator it = patterns.begin(), 
	limit = patterns.end(); it != limit; it++)
  {
    cout << it->first;
    for(unsigned int i = 0, limit = it->second.size(); i != limit; i++)
    {
      cout << " " << alphabet.getSymbol(it->second[i]);
    }
    cout << endl;
  }
...
PatternList::buildTransducer()
{
      ...
      it != limit; it++)
  {
	...
	// optional queue
        prevstate = state;
        state = transducer.insertSingleTransduction(alphabet("_") , state);
        transducer.linkStates(prevstate, state, alphabet(" "));
        transducer.linkStates(prevstate, state, alphabet("#"));
...
PatternList::write(FILE *output)
{
  ...
  string const tagger_name = "tagger";

  Compression::multibyte_write(1, output);
  Compression::multibyte_write(tagger_name.size(), output);
//  cout << tagger_name << " " << transducer.size();
//  cout << " " << transducer.numberOfTransitions() << endl;
  fwrite(tagger_name.data(), sizeof(char), tagger_name.size(), output);
...
PatternList::read(FILE *input)
{
  ...
  {
    int namelen = Compression::multibyte_read(input);
    char mystr[namelen + 1];
    fread(mystr, sizeof(char), namelen, input);
