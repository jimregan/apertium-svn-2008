...
void
TaggerData::setTagIndex(map<string, TTag> const &ti)
{
  tag_index = ti;
}
...
void
TaggerData::setArrayTags(vector<string> const &at)
{
  array_tags = at;
}
...
void
TaggerData::setPreferRules(vector<string> const &pr)
{
  prefer_rules = pr;
}
...
TaggerData::read(FILE *in)
{
  ...
  for(int i = Compression::multibyte_read(in); i != 0; i--)
  {
    int strsize = Compression::multibyte_read(in);
    char tmp[strsize+1];
    fread_unlocked(tmp, sizeof(char), strsize, in);
    tmp[strsize] = 0;
    array_tags.push_back(tmp);
  }
...
TaggerData::write(FILE *out)
{
  ...
  {
    Compression::multibyte_write(array_tags[i].size(), out);
    fwrite(array_tags[i].data(), sizeof(char), array_tags[i].size(), out);
  }

  // tag_index
  Compression::multibyte_write(tag_index.size(), out);
  for(map<string, int>::iterator it = tag_index.begin(), limit = tag_index.end();
      it != limit; it++)
  {
    Compression::multibyte_write(it->first.size(), out);
    fwrite(it->first.data(), sizeof(char), it->first.size(), out);
  ...
  {
    Compression::multibyte_write(prefer_rules[i].size(), out);
    fwrite(prefer_rules[i].data(), sizeof(char), prefer_rules[i].size(), out);
  }
