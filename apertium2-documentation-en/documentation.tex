...
Shallow-Transfer Machine Translation System \emph{Opentrad Apertium}}
%%\date{28 June 2005}
...
\bigskip
\begin{quote} Copyright \copyright 2006 Grup Transducens, Universitat
...
This documentation describes the \emph{Opentrad
Apertium}\footnote{From now on, referred to simply as
\emph{Apertium}} system, one of the open-source machine translation systems
created within the project "Open-Source Machine Translation for the
Languages of Spain" ("Traducción automática de código abierto para las
lenguas del estado español")\nota{Posem un resum de les dades del
projecte en un apèndix (codi, durada, finançament, participants, etc.)
i hi fem referència ací? - Mikel}. It is a shallow-transfer machine
translation system, initially designed for the translation between
related language pairs, although some of its components have been also
used in the deep-transfer architecture (\emph{Opentrad Matxin}) that
has been developed in the same project for the pair
Spanish-Basque. \emph{Apertium} can translate at present between the
pairs Spanish-Galician and Spanish--Catalan\footnote{With the name
\emph{Catalan} we refer also to the Valencian dialectal variant of
this language.}, and can be used to build translators between other
related language pairs, such as Danish-Swedish,Czech--Slovak, etc.
...
funded the development of this toolbox through the project
``Open-Source Machine Translation for the Languages of Spain'' of the
PROFIT program, code FIT-340101-2004-3, and its extension
FIT-340001-2005-2.
...
example, in the Romance languages dictionaries of this project, a noun
...
the source language LF to the target language LF (if they are
...
they specified.  Figure \ref{fig:pardef2} shows an example of paradigm
...
provided that these have been defined upper in the file.  In the other
...
all the inflected forms of the lemma in a morphological dictionary
...
restriction \texttt{RL} indicates that this is only a generation
...
the group of lexical forms (one or more) that are adjacent to the
...
entry (\texttt{LR} direction).  It is essential that the surface forms
...
when the system receives as input the left string of the entry (what
is between \texttt{<\textbf{l}>}) concatenated to the left string of
...
that they have the format appropriate for the dictionary compiler.
...
difference that the variable part of the lemma will be in brackets and
...
\paragraph{Processing of the metadictionary}


A metadictionary is a XML file to which two XSLT style sheets are
...
first style sheet, \texttt{buscaPar.xsl}, obtains the list of verbs
...
\end{alltt}
	...
	\texttt{<\textbf{pardef} n="m/élher\_\_vblex"/>} :
...
from dictionaries by means of a single compiler % \cite{Sergio}, based
...
their gender and number). The use of finer tags does not improve the
...
has to specify how to group the \emph{fine} tags delivered by the
...
categories, the tagger outputs fine tags like the ones from the
morphological analyser. Sometimes it may occur that the morphological
analyser delivers, for a certain word, two or more fine tags that can
be grouped under the same tagger category: e.g. in Spanish
\emph{cante} can be the 1st or the 3rd singular person of the
subjunctive present of the verb \emph{cantar} ("to sing"); both fine
...
is possible to define which fine tag, among the ones that compose a
...
specify how to group the \emph{fine} tags delivered by the
...
fine tags belong to the same category.
...
grammatical symbols (which compose the fine tag) with the output
...
with which the tagger works. Categories are defined by the fine tags
delivered by the morphological analyser in its output.
...
\item[\texttt{tags-item}]: is used to define a fine tag by means of a
...
or of fine tags.  It is designed for contractions, verbs with enclitic
pronouns, etc.

\item[\texttt{sequence}]: defines a sequence of elements, which can be
categories (\texttt{label-item}) or fine tags
(\texttt{tags-item}). Using fine tags directly is useful if one wishes
...
which fine tag must be delivered in the tagger output when two or more
fine tags are assigned to the same category.
  
\item[\texttt{prefer}]: specifies that, in case of conflict between
different fine tags assigned to the same category, the tagger must
...
format of the needed text can be automatically obtained from a plain
...
from enclitic to proclitic and vice versa.
...
dictionary has to store an entry for each of the different
combinations.  By separating compound multiwords we can avoid having
...
by the generator, is left to the appropriate rules of the transfer
module, which are also responsible for creating the compound
multiwords which may be needed in the target language.  In general,
...
languages as the ones dealt with in the first stages of the engine,
...
difference in meaning between the multiple equivalents in target
...
add a direction restriction in the rest of translations.  In the other
...
have any lexical selection module, a style sheet is responsible for
converting these entries designed for a lexical selection module into
...
that must be done on a bilingual dictionary that contains more than
one translation per entry, regardless of whether the system uses a
lexical selector or not; finally, Section~\ref{se:lextor} describes
...
fact makes it necessary to pre-process those dictionaries, since the
...
section provide the information necessary to perform lexical selection
...
\nota{Faena per fer (mlf):
  \begin{itemize}
  \item El document no diu encara res de metaparadigmes. En cal una
  descripció.
  \item Caldria demanar comentaris a Marta Villegas com a usuària del
  document perquè ens indique parts fosques, possibles suggeriments,
  etc.
...
Since 2007, Apertium incorporates a structural transfer system more
advanced than the one used previously; its need came with the
development of machine translators for less related language pairs in
comparison with the ones dealt with before, for example, for the
language pair \emph{English}--\emph{Catalan}.

This enhanced transfer system is made of three modules, the first one
of which can be used in isolation if the user wants to run a
\textbf{shallow-transfer} system (which is the transfer system used so
far for related language pairs such as {Spanish}--\emph{Catalan} or
\emph{Spanish}--\emph{Galician}).  When the system is used for less
related language pairs and, therefore, we wish to perform an
\textbf{advanced transfer}, the three transfer modules will be run.
...
We describe next the characteristics of this module.  Section
...
advanced transfer system, with the differences presented in that
...
explains the required pre-processing of the modules.
...
The transfer process is organised around patterns representing
...
sequence follows a certain pattern if it contains the lexical forms
sequence of the pattern.  Patterns do not need to be constituents or
...
certain language is selected so that the most common structural
transformations are covered.  When source language and target language
are syntactically similar, as is the case of Spanish, Catalan and
...
catalogue, and processes them applying the suitable structural
transfer rules, doing at the same time the lexical transfer by reading
...
position. This is done by using, in output of the rule, the attributes
...
$w_i=$\verb!^!$l_{i}E_{i}$\verb!$!  contains lemma $l_i$ and
  morph. symbols $E_i$, some of which can be \emph{references or links
...
category and will be detected by patterns.  It should be emphasized
that the linguist may include any information about the lexical form
...
lists of lemmas, which will be used to search in them a certain value
...
(i.e. \texttt{det}, \texttt{adv}, \texttt{prep}, etc.) and a list of
...
In this section, the attributes are described which will be extracted
...
\end{alltt}
\end{small}
\caption{Call to the macro-instruction \texttt{f-concord2} designed to
...
the king \texttt{<\textbf{choose}>}, \texttt{<\textbf{let}>},
...
search any value (specified as the first parameter of the condition)
...
es \texttt{yes}, the search is done ignoring case. Figure \ref{fig:in}
...
TL lexical form, defined by the value of different attributes (see an
...
\caption{Use of the element \texttt{<\textbf{lit-tag}>}: it is tested
  whether the number (\texttt{nbr}) tag (or symbol) of the second
...
\texttt{<section-def-lists>}. An example of its use can be found in
Figure \ref{fig:in}.
...
values "\texttt{aa}", "\texttt{Aa"} o "\texttt{AA}". It works like the
...
same in the SL and in the TL and, therefore, it is automatically
...
\end{alltt}
\end{small}
\caption{Extract from a rule for the pattern determiner--noun
...
\caption{Output instruction that sends two chunks separated by a
  ...
  phrase ("NP"). The tags that are linked outside the chunk are
...
\item \textbf{Use in \texttt{interchunk}}
  ...
  The information on lemma and tags inside a \texttt{<\textbf{chunk}>}
  element refer exclusively to the lemma (pseudolemma) and the tags of
  the chunk.
...
\label{ss:lu} This is the element by means of which each TLLF is sent
in the output of a rule, inside an \texttt{<\textbf{out}>} element.
...
It is important to remember that, as explained before, in the case of
...
position 1 and 2 (determiner and noun). Contrarily, in the first
...
This is the element in which chunks are sent, in a
...
The root element (\texttt{<transfer>}) of the chunker rules file
\texttt{.t1x} has an optional attribute \texttt{default} which can
take the values
\begin{itemize}
\item \texttt{chunk}: when the module works in advanced mode (advanced
transfer system): it will generate trivial chunks with the words not
matched by any rule, so that in the output there are no words not
included in a chunk.  The new chunk will be created with the
translation of the word by the bilingual dictionary.  The fixed lemma
of these implicitly created chunks is \texttt{default}.
\item \texttt{lu} (default value): when the module works as single
...
the \texttt{chunker} module; they are written, however, a different
...
the pseudolemma and the tags of each chunk.
...
extension \texttt{.t3x}. Syntax is based as well in the
...
written, since chunk matching is done individually (this module does
not group chunks).
...
\textit{extensive superblanks}.  The encapsulate long format fragments
...
<head> 
<title>]}This is the
title\textbf{.[][@/tmp/temp35345]}This\textbf{[ 
...
encodings, if case must be considered, and the regular expressions for
...
de-formatter. These tags may, optionally, indicate a sentence end so
that the de-formatter inserts an artificial punctuation mark (followed
by an empty block of format, as explained in the previous
section). One has to specify the rules' application priority,
...
\texttt{rtf-format.xml}, \texttt{txt-format.xml}.  As a matter of
fact, it is quite simple to define de-formatters and re-formatters for
any XML format.
...
(\texttt{apertium-}$LANG_1$\texttt{-}$LANG_2$\texttt{-}$\textrm{VERSION}$\texttt{.tar.gz})
...
(@, \# o /); if you wish that neither unknown words nor errors are
...
sentences to this text (and to \texttt{es.tagged.txt}, used to
generate automatically the corpus \texttt{es.untagged}) in order to
...
tool to enhance linguistic data.
...
following syntax: \texttt{paradigmes-ll-rr} , where \textit{ll} and
...
part of speech of the entry being inserted. When a new paradigms is
...
format. This question will be explained in more detail in Section
...
language to source language.\nota{MG: ídem ("bilingual"). Also,
confusion with TL and SL, I think it's not clear.}
\end{itemize}
...
allows us to delete the current entry if we decide not to validate and
insert it.
...
of entries made by non-professional users.  If the chose action is a
...
(\texttt{\$cad\_bil}). \nota{MG: I think speaking of SL and TL is
confusing, since translation works in both directions} Then, the
...
screen is completed adding the buttons for accepting, deleting and go
...
\nota{Intentar evitar el llenguatge sexista en tot el document:
\emph{usuari}, etc.}  When a user wants to insert new entries in a
...
define the entries. If the entry already exists in any of the
dictionaries, a warning is displayed and the system automatically
...
Professional users have two additional links in the screen for the
...
\newpage
\subsection{Especification of lexical forms}
