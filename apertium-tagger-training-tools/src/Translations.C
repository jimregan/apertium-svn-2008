...
void
Translations::set_path_translation(string trans, int path) {
  /*
  ...
  */
  ...
  int index_trans=translations[trim(trans)];
  ...
  if ((segment->get_number_paths()>1) && (trim(trans).length()==0))
    all_translations_ok=false;
}
...
Translations::get_path_translation(int path) {
  if (path_translation.find(path)!=path_translation.end())
    ...
    cerr<<"Warning:Translations::get_path_translation wass called but there is no translation for the path "<<path<<", returning empty string\n";
    return "";
  }
}
...
Translations::set_translation_likelihood(int index_trans, double likelihood) {
  ...
  if (isnan(likelihood))
    cerr<<"Error: Likelihood of translation '"<<translations[index_trans]<<"' in NAN\n";
  if (isinf(likelihood))
    cerr<<"Error: Likelihood of translation '"<<translations[index_trans]<<"' in INF\n";
}
