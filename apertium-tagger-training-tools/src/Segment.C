...
void 
Segment::set_tag_index(map<string, TTag> ti) {
  tag_index=ti;
}
...
Segment::new_segment(MorphoStream &ms, TransferRules* tr) {
  ...
  if ((index_busca_corte<=0) || (index_busca_corte>=palabras_frase.size())) {
	...
	continuar=false;
      ...
      if ((tags.size()==1) && (((*tags.begin())==tag_index["TAG_SENT"]) ||
			       ((*tags.begin())==tag_index["TAG_kEOF"]) ||
			       ((*tags.begin())==tag_index["TAG_LQUEST"]))) {
  ...
  for(int i=index_busca_corte; i<palabras_frase.size(); i++) {
    ...
    if (tr->is_segmentation_point(tag_index["TAG_kEOF"], palabras_frase, i, avance)) {
      //cerr<<"yes\n";
  ...
  if ((punto_corte==-1)&&(palabras_frase.size()>0)) {
    cerr<<"Error: (grave) No se encontró ningún punto de corte en la frase\n";
    cerr<<"Long frase tratada: "<<palabras_frase.size()<<"\n";
    cerr<<"FRASE: ";
    for(int i=0; i<palabras_frase.size(); i++)
      cerr<<palabras_frase[i]->get_superficial_form()<<" ";
    cerr<<"\n";   
