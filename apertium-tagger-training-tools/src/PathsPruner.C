...
PathsPruner::a_priori_likelihood(int path) {
  ...
  //    cerr<<tagger_data.getArrayTags()[tagseq[i]]<<" ";
  //  }
  //  cerr<<"\n";
  //}
  ...
  if (seg->vwords_before.front().get_tags().size()!=1) {
    ...
    for(int i=0; i<seg->vwords_before.size(); i++) {
      cerr<<seg->vwords_before[i].get_superficial_form()<<" ";
    }
    exit(EXIT_FAILURE);
  } 
  ...
  if (seg->vwords_after.back().get_tags().size()!=1) {
    ...
    for(int i=0; i<seg->vwords_after.size(); i++) {
      cerr<<seg->vwords_after[i].get_superficial_form()<<" ";
    }
    exit(EXIT_FAILURE);
  } 
  ...
  for (int i=1; i<seg->vwords_before.size(); i++) {
    ...
    //if(Utils::debug) {
    //  for (itag=tags.begin(); itag!=tags.end(); itag++)
    // 	  cerr<<"alpha["<<i<<"]["<<tagger_data.getArrayTags()[*itag]<<"]="<<alpha[i][*itag]<<"\n";
    //}
  ...
  //if(Utils::debug) {
  //  for (jtag=tags.begin(); jtag!=tags.end(); jtag++)
  //    cerr<<"beta[0]["<<tagger_data.getArrayTags()[*jtag]<<"]="<<beta[0][*jtag]<<"\n";
  //}
  ...
  for (int i=1; i<seg->vwords_after.size(); i++) {
    ...
    //if(Utils::debug) {
    //  for (itag=tags.begin(); itag!=tags.end(); itag++)
    //	  cerr<<"beta["<<i<<"]["<<tagger_data.getArrayTags()[*itag]<<"]="<<beta[i][*itag]<<"\n";
    //}
